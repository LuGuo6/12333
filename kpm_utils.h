...
/*
 * KPM 工具库 - 所有 KPM 模块的公共基础设施
 * 基于 KernelPatch hook 框架封装
 *
 * 核心能力:
 *   1. lookup_name()  - 通过 kallsyms 查找内核函数地址
 *   2. hook_func()    - 对内核函数进行 before/after 拦截
 *   3. unhook_func()  - 解除 hook
 *   4. kfunc()/kvar() - 调用内核函数 / 访问内核全局变量
 *   5. task_uid()等   - 获取进程 UID/GID
 *   6. ARM64指令解析   - 动态计算内核结构体偏移量
 */
#ifndef _KPM_UTILS_H
#define _KPM_UTILS_H
...
// 通过 task_struct 和 cred 偏移量获取进程凭据
